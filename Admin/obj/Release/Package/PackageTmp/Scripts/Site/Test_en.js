...
app.controller('TestAPIController', function ($scope, ContactService, $timeout) {
    ...
    $scope.addQuestion = function (id) {
        ...
        if ($scope.question_form.Question == "") {
            $scope.add_error = "Question can not be empty."
            $scope.clear($scope);
            return;
        }
        if ($scope.question_form.Points == null || $scope.question_form.Points == "") {
            $scope.add_error = "Points can not be empty."
            $timeout(function () {
                $scope.add_error = "";
            }, 3000);
            return;
        ...
        if (!flag) {
            $scope.add_error = "One of answers should be correct."
            $timeout(function () {
                $scope.add_error = ""
            }, 3000);
        }
    ...
    $scope.editQuestion = function (question) {
        ...
        if (flag)
            ContactService.editQuestion(question).then(function () {
                $scope.successful = "Data successful updated"
                $timeout(function () {
                    $scope.successful = ""
                }, 3000);
            });
        else {
            $scope.edit_error = "One of answers should be correct."
            $timeout(function () {
                $scope.edit_error = ""
            }, 3000);
        }
